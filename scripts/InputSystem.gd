...
@onready var command_system: CommandSystem = get_node("/root/CommandSystem")
@onready var unit_manager: UnitManager = get_node("/root/UnitManager")
@onready var globe: Globe = get_node("/root/Main/Globe")
...
func _raycast_to_globe(mouse_pos: Vector2, cam: Camera3D) -> Vector3:
	...
	return globe._raycast_to_globe_sphere(from, dir) if globe.has_method("_raycast_to_globe_sphere") else Vector3.ZERO
