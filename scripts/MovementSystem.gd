...
func request_move(entity: Node, target_world_pos: Vector3) -> bool:
	...
	var requires_water := is_naval

	var globe := entity.get_parent() if entity.has_method("get_globe") else entity.get_parent()
	...
	var path := generate_domain_aware_path(entity, target_world_pos, etype)
...
func generate_domain_aware_path(entity: Node, target: Vector3, etype: String) -> Array[Vector3]:
	...
	var direct_path := _generate_direct_path(start, target)
