...
func create_states() -> void:
	var data: Dictionary = load_geojson("res://data/geojson/states.geojson")
	...
	for feature_variant in data.get("features", []):
		...
		# Store polygon data for LandSystem
		var geometry: Dictionary = feature.get("geometry", {})
		var gtype: String = str(geometry.get("type", ""))
		if gtype == "Polygon" or gtype == "MultiPolygon":
			var coords_var = geometry.get("coordinates", [])
			var coords: Array = coords_var if coords_var is Array else []
			var outer: Array = []
			if gtype == "Polygon" and coords.size() > 0:
				outer = coords[0] if (coords[0] is Array) else []
			elif gtype == "MultiPolygon" and coords.size() > 0:
				var first = coords[0] if (coords[0] is Array) else []
				if first.size() > 0:
					outer = first[0] if (first[0] is Array) else []
				if outer.size() > 0:
